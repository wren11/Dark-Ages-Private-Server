    ...
    public class MetafileManager
    {
        ...
        private static void LoadQuestDescriptions()
        {
            ...
            foreach (var file in Directory.GetFiles(dir, "*.txt"))
            {
                ...
                var nodes = StorageManager.Deserialize<List<Node>>(contents);
            ...
            {
                var metaFile = new Metafile {Name = $"SEvent{i}", Nodes = new List<MetafileNode>()};
        ...
        private static void GenerateClassMeta()
        {
            ...
            foreach (var template in from v in ServerContext.GlobalSkillTemplateCache
                ...
                select v.Value)
            {
                if (template.Prerequisites != null && template.Prerequisites.Class_Required == Class.Warrior)
                    sclass1.Nodes.Add(new MetafileNode(template.Name, template.GetMetaData()));

                if (template.Prerequisites != null && template.Prerequisites.Class_Required == Class.Rogue)
                    sclass2.Nodes.Add(new MetafileNode(template.Name, template.GetMetaData()));

                if (template.Prerequisites != null && template.Prerequisites.Class_Required == Class.Wizard)
                    sclass3.Nodes.Add(new MetafileNode(template.Name, template.GetMetaData()));

                if (template.Prerequisites != null && template.Prerequisites.Class_Required == Class.Priest)
                    sclass4.Nodes.Add(new MetafileNode(template.Name, template.GetMetaData()));

                if (template.Prerequisites != null && template.Prerequisites.Class_Required == Class.Monk)
                    sclass5.Nodes.Add(new MetafileNode(template.Name, template.GetMetaData()));
            }
            ...
            foreach (var template in from v in ServerContext.GlobalSpellTemplateCache
                let prerequisites = v.Value.Prerequisites
                where prerequisites != null
                orderby prerequisites.ExpLevel_Required
                select v.Value)
