        ...
        where TClient : NetworkClient<TClient>, new()
    {
        ...
        private void EndConnectClient(IAsyncResult result)
        {
            try
            {
                ...
                {
                    Network = new NetworkSocket((Socket)socket),
                };
                ...
                if (client.Network.ConnectedSocket.Connected)
                    if (AddClient(client))
                    {
                        ...
                        {
                            Console.WriteLine("[Network Server]: error {0} in {1}", error, "EndConnectClient");
                    ...
                    {
                        Console.WriteLine("[Network Server]: Client could not be added.");
                        ClientDisconnected(client);
                    }
        ...
        private void EndReceiveHeader(IAsyncResult result)
        {
            try
            {
                if (result.AsyncState is TClient client)
                {
                    ...
                    {
                        Console.WriteLine("[Network Server]: error {0} in {1}", error, "EndReceiveHeader");
            ...
            try
            {
                if (result.AsyncState is TClient client)
                {
                    ...
                    if (bytes == 0 ||
                        error != SocketError.Success)
                    {
                        Console.WriteLine("[Network Server]: error {0} in {1}", error, "EndReceivePacket");
                        ClientDisconnected(client);
                        return;
                    }
        ...
        public virtual void ClientDisconnected(TClient client)
        {
            ...
            {
                Console.WriteLine("Socked Closed: ", client.Network.ConnectedSocket.Handle);
//                client.Network.ConnectedSocket.Disconnect(false);
        ...
        private void RemoveAisling(TClient client)
        {
            ...
            var nearby = GetObjects<Aisling>(i => i.WithinRangeOf((client as GameClient).Aisling));
