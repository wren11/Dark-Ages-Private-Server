...
namespace Darkages.Network.Login
{
    public class LoginServer : NetworkServer<LoginClient>
    {
        public LoginServer(int capacity)
            : base(capacity)
        {
            MServerTable = MServerTable.FromFile(Path.Combine(Environment.CurrentDirectory, "Static", "MServerTable.xml"));
            Notification = Notification.FromFile(Path.Combine(Environment.CurrentDirectory, "Static", "notification.txt"));
        }
